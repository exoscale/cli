...
func (c *dbaasUserResetCmd) resetRedis(cmd *cobra.Command, _ []string) error {
	...
	req := v3.ResetDBAASRedisUserPasswordRequest{}
	if req.Password != "" {
		req.Password = v3.DBAASUserPassword(c.Password)
	}

	op, err := client.ResetDBAASRedisUserPassword(ctx, c.Name, c.Username, req)
